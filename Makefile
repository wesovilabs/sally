PROJECT  = meerkat
PACKAGE  = github.com/wesovilabs/sally
DOCKER_REGISTRY = wesovilabs/sally
...
.PHONY: release
release: | $(BASE) ; $(info $(M) Building docker images…) @ ## Build docker images
	@echo "Building meerkat-cli..."
	$Q cd $(BASE) && GOOS=linux GOARCH=amd64 go build \
    	-tags release \
		-o build/meerkat-cli-$(VERSION) main.go  &&\
		docker build -f Dockerfile -t $(DOCKER_REGISTRY)/meerkat-cli:$(VERSION) .   &&\
		docker image tag $(DOCKER_REGISTRY)/meerkat-cli:$(VERSION) $(DOCKER_REGISTRY)/meerkat-cli:latest \


.PHONY: publish
publish: release | $(BASE) ; $(info $(M) publishing dcker image) @ ## Publish docker image
	@echo "Publishing meerkat-cli..."
	$Q cd $(BASE) && docker login -u $(DOCKER_USER) -p $(DOCKER_PASS) docker push $(DOCKER_REGISTRY)/meerkat-cli:$(VERSION)

.PHONY: run
run: vendor  | $(BASE) ; $(info $(M) Running store-mongodb) @ ## Run integration tests for module store-mongodb
